...
def lk(request):
    ...
    image = Users.objects.get(number = user).image
...
def reg_form(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if type == "Пользователь": return redirect('prof')
            elif type == "Жюри": return redirect('prof')
            elif type == "Организатор": return redirect('org')
...
def profile(request):
    user = request.user
    user = Users.objects.get(number = user)
    if user.type != "Организатор":
        cards = GetCards(Event.objects.filter(title = user.event))
    else:
        cards = ["<h5 class='m-1'>Просмотреть свои мероприятия вы можете <a href='/lk/'>Здесь</a></h5>"]
    data = {"NAME": user.full_name, "MAIL":user.email, "TEL": user.phone, "ID": user.number, "TYPE": user.type, "cards" : cards, "IMG": f"/{str(user.image)[6:]}"}
